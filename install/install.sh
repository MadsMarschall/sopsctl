...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t sopsctl-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/sopsctl.json"
    TMP_HASH="${TMP_DIR}/sopsctl.hash"
    TMP_BIN="${TMP_DIR}/sopsctl.tar.gz"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing sopsctl to ${BIN_DIR}/sopsctl"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/sopsctl\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
